...
let server = "https://equity-knights-beef-basically.trycloudflare.com"
...
function createBundle(BUNDLE_DATA) {
  ...
  BUNDLE_DATA.forEach((el, bundleIndex) => {
    ...
    if (el.type == "productBundle") {
      ...
      console.log("Check enter in product mix match.....................",el);
      let OriginalPrice = 0;
      let FinalPrice = 0;
      let PriceArray = [];
          ...
          SelectVariants.addEventListener("change",VariantOptionFun=(e)=>{
            ...
            elements.variants.map((variants,variantIndex)=>{
              SelectedProducts.map((item,itemIndex)=>{
                // if(item.length >1){
                  console.log("enter in if of  select")
                  item.map((itemProducts,indexOfItemProduct)=>{
                    if(variants.id == itemProducts){
                    console.log("enter in if of multi select",itemProducts,"===",variants.id);
                      indexOfProduct = SelectedProducts.findIndex((checkPrducts)=> checkPrducts[indexOfItemProduct] == variants.id)
                    }
                  })
                // }else{
                //   console.log("enter in else of  select")
                //   if(variants.id == item){
                //     indexOfProduct = SelectedProducts.findIndex((checkPrducts)=> checkPrducts == variants.id)
                //   }
                // }
              })
            })
            console.log("check index of price",indexOfPrice,"index of products",indexOfProduct);
            ...
            console.log("check......... variants dropdown SelectedProducts Array =====******>>>>>",SelectedProducts);
            console.log("check..... variants dropdown priceArray =====******>>>>>",PriceArray);
         ...
         el.bundleDetail.discountOptions.map((item,index)=>{
          if(SelectedProductsQuantity < el.bundleDetail.discountOptions[0].quantity ){
            ...
            if(appliedDiscount !==null && PriceDiv !== null){
              appliedDiscount.innerText = `Select at least ${el.bundleDetail.discountOptions[0].quantity} items to apply the discount`;
            ...
            let PriceDiv = document.getElementById(`${bundleIndex}OriginalPrice`);
            if(appliedDiscount !==null && PriceDiv !== null){
              appliedDiscount.innerText = "No discount is applied on the selected products";
              PriceDiv.style.display = "none";
            }
            FinalPrice = OriginalPrice;
            return FinalPrice.toFixed(2);
            ...
            if(el.bundleDetail.discountOptions[index].type === "percent"){
              if((item.quantity == SelectedProductsQuantity)  || (index === el.bundleDetail.discountOptions.length-1 && SelectedProductsQuantity >= item.quantity)) {
                ...
                let PriceDiv = document.getElementById(`${bundleIndex}OriginalPrice`);
                if(appliedDiscount !==null && PriceDiv !== null){
                  appliedDiscount.innerText = `${item.value}% ${el.translations.translation.discountApplied}.`;
              ...
              if((item.quantity == SelectedProductsQuantity)  || (index === el.bundleDetail.discountOptions.length-1 && SelectedProductsQuantity >= item.quantity)) {
                ...
                let PriceDiv = document.getElementById(`${bundleIndex}OriginalPrice`);
                if(appliedDiscount !==null && PriceDiv !== null){
                  appliedDiscount.innerText = `-${showAmountWithCurrency(item.value)} ${el.translations.translation.discountApplied}.`;
                  PriceDiv.style.display = "block";
                }
                if(FinalPrice <0){
                  FinalPrice = 0;
              ...
              if((item.quantity == SelectedProductsQuantity)  || (index === el.bundleDetail.discountOptions.length-1 && SelectedProductsQuantity >= item.quantity)) {
                ...
                let PriceDiv = document.getElementById(`${bundleIndex}OriginalPrice`);
                if(appliedDiscount !==null && PriceDiv !== null){
                  appliedDiscount.innerText = `100% ${el.translations.translation.FreeShipping} ${el.translations.translation.discountApplied}.`;
                  PriceDiv.style.display = "none";
                }
                return FinalPrice.toFixed(2)
              }
              ...
              if((item.quantity == SelectedProductsQuantity)  || (index === el.bundleDetail.discountOptions.length-1 && SelectedProductsQuantity >= item.quantity)) {
                ...
                let appliedDiscount = document.getElementById(`${bundleIndex}discountOffer`);
                if(appliedDiscount !==null && PriceDiv !== null){
                  appliedDiscount.innerText = `${el.translations.translation.discountApplied}.`;
                  PriceDiv.style.display = "none";
                }
                return FinalPrice.toFixed(2)
              }
      ...
      ShowSelectedItemDiv.innerText = `You have selected ${el.bundleDetail.products.length} items`;
      ...
      AllProductCheckBox.addEventListener("change",handleAllProducts = (e)=>{
        if(e.target.checked === false){
          if(el.bundleDetail.requiredItem.enable === false){
            ...
            document.getElementById(`${bundleIndex}selectedItems`).innerText = `You have not selected any items yet`;
            ...
            el.bundleDetail.products.map((requireItem,requiredItemIndex)=>{
              ...
              calaculateOriginalPrice();
              calculateFinalPriceFunction();
              document.getElementById(`${bundleIndex}selectedItems`).innerText = `You have selected ${SelectedProductsQuantity} items`;
              document.getElementById(`${bundleIndex}QuantityCounter`).innerText = `${SelectedProductsQuantity}X`;
              document.getElementById(`${bundleIndex}OriginalPrice`).innerText = showAmountWithCurrency(OriginalPrice.toFixed(2));
              document.getElementById(`${bundleIndex}FInalPrice`).innerText = showAmountWithCurrency(FinalPrice.toFixed(2));
            })
      ...
      AllCheckBoxLabel.innerText = "All Products";
      ...
      ItemCounterDiv.append(HorizontalLine3);
      ...
      el.bundleDetail.products.map((elements,productIndex)=>{
        ...
        // bundleData[bundleIndex][productIndex] = {
        //   id: elements.variants[0].id.replace(
        //     "gid://shopify/ProductVariant/",
        //     ""
        //   ),
        //   quantity: 1,
        // };
        // console.log("check the element bundle data",bundleData);
        let TotalQuantity = 1;
        SelectedProductsQuantity = SelectedProducts.length;
          ...
          MinusBtn.addEventListener("click",removeProduct = ()=>{
            ...
            elements.variants.map((variants,variantsIndex)=>{
              SelectedProducts.map((item,itemIndex)=>{
                ...
                // }else{
                //   if(variants.id == item){
                //     indexofProducts = SelectedProducts.findIndex((checkPrducts)=> checkPrducts == variants.id)
                //   }
                // }
              })
            })
            ...
            calaculateOriginalPrice();
            calculateFinalPriceFunction();
            console.log("check all price array and product array",PriceArray,"product array",SelectedProducts)
            if(TotalQuantity <= 1){
              document.getElementById(`Minusbtn${bundleIndex}${productIndex}`).disabled = true;
            }
            document.getElementById(`${bundleIndex}OriginalPrice`).innerText = showAmountWithCurrency(OriginalPrice.toFixed(2));
          ...
          PlusBtn.addEventListener("click",addProduct = ()=>{
            ...
            elements.variants.map((variants,variantsIndex)=>{
              SelectedProducts.map((item,itemIndex)=>{
                ...
                // }
                // else{
                  console.log("enter in else of multi select",indexofProducts)
                //   if(variants.id == item){
                //     indexofProducts = SelectedProducts.findIndex((checkPrducts)=> checkPrducts == variants.id)
                //   }
                // }
            ...
            let VariantOptionDiv = document.getElementById(`VariantOptionDiv${bundleIndex}${productIndex}`)
            console.log(".......................................54546666565",VariantOptionDiv)
            SelectedProducts[indexofProducts].push(elements.variants[0].id);
            TotalQuantity =  SelectedProducts[indexofProducts].length;
            VariantDropDownFun(productIndex,elements,VariantOptionDiv,TotalQuantity);
            console.log("check Selected product in multi select product.........",SelectedProducts);
            console.log("check Price array in multi select product.........",PriceArray);
      ...
      AddToCartBTN.addEventListener("click",addToCartFunc=(e)=>{
          // console.log("checkprice array ====>>>>>>>",item);
        console.log('check discountType....',DiscountType,'discountValue...................',DiscountValue)
        ...
        // console.log("push all id in bundle data array=====>>>>",bundleSum);
        let code = JSON.parse(localStorage.getItem("sd_bundle_data"));
        let discountCodeId = code.discountCreateId ?? "";
        AddToCartBTN.innerText = "";
        let btnSpinner = document.createElement("div");
        ...
        // console.log("hello this is bundle data=====>>>>>>>",bundleData[bundleIndex]);
        const mergedGidArray = SelectedProducts.reduce(
          (acc, curr) => acc.concat(curr),
          []
        );
        // console.log("mergedGidArraymergedGidArraymergedGidArraymergedGidArraymergedGidArraymergedGidArray",mergedGidArray)
        let mergedResult = {};
        for (let item of mergedArray) {
          if (mergedResult[item.id]) {
            mergedResult[item.id].quantity += item.quantity;
      ...
      el.bundleDetail.discountOptions.map((ele,index)=>{
        ...
        if(el.bundleDetail.discountOptions[index].type === "percent"){
          if((ele.quantity == SelectedProductsQuantity)  || (index === el.bundleDetail.discountOptions.length-1 && SelectedProductsQuantity >= ele.quantity)) {
            getDiscountTextDiv.innerText = `${ele.value}% ${el.translations.translation.discountApplied}.`;
            activeBadge.className = "sd-bundle-mix-match-active-badge-div";
          }
        }
        if(el.bundleDetail.discountOptions[index].type === "fixed"){
          if((ele.quantity == SelectedProductsQuantity)  || (index === el.bundleDetail.discountOptions.length-1 && SelectedProductsQuantity >= ele.quantity)) {
            getDiscountTextDiv.innerText = `-${showAmountWithCurrency(ele.value)} ${el.translations.translation.discountApplied}.`;
            activeBadge.className = "sd-bundle-mix-match-active-badge-div";
          }
        }
        if(el.bundleDetail.discountOptions[index].type === "noDiscount"){
          if((ele.quantity == SelectedProductsQuantity)  || (index === el.bundleDetail.discountOptions.length-1 && SelectedProductsQuantity >= ele.quantity)) {
            getDiscountTextDiv.innerText = `${el.translations.translation.discountApplied}.`;
            activeBadge.className = "sd-bundle-mix-match-active-badge-div";
            PriceDiv.style.display = "none";
          }
        }
        if(el.bundleDetail.discountOptions[index].type === "freeShipping"){
          if((ele.quantity == SelectedProductsQuantity)  || (index === el.bundleDetail.discountOptions.length-1 && SelectedProductsQuantity >= ele.quantity)) {
            getDiscountTextDiv.innerText = `100% ${el.translations.translation.FreeShipping} ${el.translations.translation.discountApplied}.`;
            activeBadge.className = "sd-bundle-mix-match-active-badge-div";
            PriceDiv.style.display = "none";
          }
        }
